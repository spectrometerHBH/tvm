...
bool IsScalarBufferAccess(const Buffer& buffer, const ffi::Array<PrimExpr>& indices) {
 ...
 private:
  void VisitStmt_(const BufferStoreNode* store) final {
    ...
    if (const auto* call = store->value.as<CallNode>()) {
      ...
      is_token_value = buffers_->count(load->buffer->data.get());
    }
    if (is_token_value && buffers_->insert(store->buffer->data.get()).second) changed = true;
 ...
 private:
  void VisitStmt_(const BufferStoreNode* store) final {
    ...
    if (const auto* call = store->value.as<CallNode>(); call && IsTokenProducer(call)) {
      ...
      auto it = declarations_->find(load->buffer->data.get());
      if (it != declarations_->end()) possible = it->second;
    }
    if (!possible.empty()) {
      auto& target = (*declarations_)[store->buffer->data.get()];
...
class RangeEndSchemaVerifier : public StmtExprVisitor {
 ...
 private:
  void VisitExpr_(const CallNode* call) final {
    ...
    auto possible_it = token_declarations_.find(token->buffer->data.get());
...
class TokenVerifier : public StmtExprVisitor {
 ...
 private:
  void VisitStmt_(const BufferStoreNode* store) final {
    if (!token_buffers_.count(store->buffer->data.get())) {
      StmtExprVisitor::VisitStmt_(store);
      return;
    }
    ...
    if (const auto* call = store->value.as<CallNode>()) {
      ...
      valid_value = token_buffers_.count(load->buffer->data.get());
  ...
  void VisitExpr_(const BufferLoadNode* load) final {
    if (token_buffers_.count(load->buffer->data.get())) {
  ...
  void VisitExpr_(const CallNode* call) final {
    ...
    if (call->op.same_as(IketRangeEndOp())) {
      ...
      const auto* token = call->args[0].as<BufferLoadNode>();
      TVM_FFI_CHECK(token != nullptr && token_buffers_.count(token->buffer->data.get()), ValueError)
...
class OfficialTokenAnalyzer {
 ...
 private:
    ...
    for (const TokenAnalysisState& old_state : input) {
      ...
      if (const auto* call = store->value.as<CallNode>(); call && IsTokenProducer(call)) {
        if (call->op.same_as(IketSentinelOp())) {
          state.token_values[store->buffer->data.get()] = TokenValue{TokenValueKind::kSentinel, {}};
        } else {
          std::string name = GetName(call);
          ...
          state.token_values[store->buffer->data.get()] =
              TokenValue{TokenValueKind::kActiveRange, std::move(name)};
        }
      } else if (const auto* load = store->value.as<BufferLoadNode>();
                 load && token_buffers_.count(load->buffer->data.get())) {
        auto source = state.token_values.find(load->buffer->data.get());
        if (source == state.token_values.end()) {
          valid_ = false;
          return {};
        }
        state.token_values[store->buffer->data.get()] = source->second;
      } else {
        valid_ = false;
        return {};
      }
  ...
  TokenAnalysisStates ProcessRangeEnd(const CallNode* call, const TokenAnalysisStates& input) {
    ...
    if (!load || !token_buffers_.count(load->buffer->data.get())) {
      valid_ = false;
      return {};
    }
    TokenAnalysisStates result;
    for (const TokenAnalysisState& old_state : input) {
      auto token = old_state.token_values.find(load->buffer->data.get());
      ...
      state.token_values[load->buffer->data.get()] = TokenValue{TokenValueKind::kConsumed, {}};
      result.insert(std::move(state));
    }
    return Limit(std::move(result));
  }
...
class StripIket : public StmtExprMutator {
 ...
 private:
  Stmt VisitStmt_(const AllocBufferNode* alloc) final {
    if (token_buffers_.count(alloc->buffer->data.get())) return Evaluate(0);
    return StmtExprMutator::VisitStmt_(alloc);
  }

  Stmt VisitStmt_(const BufferStoreNode* store) final {
    if (token_buffers_.count(store->buffer->data.get())) return Evaluate(0);
...
class UniformExprChecker : public ExprVisitor {
 ...
 private:
  ...
  void VisitExpr_(const BufferLoadNode* load) final {
    if (!uniform_buffers_.count(load->buffer->data.get()) ||
...
class IketConvergenceVerifier : public StmtExprVisitor {
 private:
  ...
  void VisitStmt_(const BufferStoreNode* op) final {
    ...
    VisitExpr(op->value);
    if (uniform_store) {
      uniform_buffers_.insert(op->buffer->data.get());
    } else {
      uniform_buffers_.erase(op->buffer->data.get());
    }
  }
