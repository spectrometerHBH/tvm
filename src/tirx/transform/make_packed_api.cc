...
PrimFunc MakePackedAPI(PrimFunc func) {
  ...
  ffi::Array<ffi::String> nullable_buffer_params;
  if (auto opt = func->GetAttr<ffi::Array<ffi::String>>(tirx::attr::kNullableBufferParams)) {
    nullable_buffer_params = opt.value();
  }
  TVMFFIABIBuilder binder(name_hint, func_ptr->params, func_ptr->buffer_map, nullable_buffer_params,
