...
class PipelineOpaqueAccessRewriter {
 public:
  ...
  PrimExpr Rewrite(const Call& call) {
    ...
    static const Op& ptx_ldmatrix = Op::Get("tirx.ptx_ldmatrix");
    static const Op& ptx_mma = Op::Get("tirx.ptx_mma");
...
class PipelineRewriter : public StmtExprMutator {
 ...
 private:
  ...
  Stmt EmitImpl(PrimExpr start, PrimExpr end, bool unroll_loop,
    ...
    PopulateWaitCounts(new_blocks, ana_normalized.get(), buffer_to_commit_group, &async_states_local);
    auto stmts = CompletePipelineLoopStatements(new_blocks, async_states_local, ana_normalized.get());
