...
def _tcgen05_cast_warpgroup_kernel():
    ...
    @T.prim_func
    def kernel(
        a_bf16: T.Buffer((_TCGEN05_M, _TCGEN05_K), "bfloat16", scope="local", layout=atom_layout),
        a_fp32: T.Buffer((_TCGEN05_M, _TCGEN05_K), "float32", scope="local", layout=atom_layout),
    ) -> None:
        T.device_entry()
        T.cta_id([1])
        T.warpgroup_id([1])
...
def test_elementwise_reg_tcgen05_slice_canonicalize_under_sctx_target():
    """B00011: elementwise reg predicate must slice+canon under ``sctx.target``.

    tcgen05 ``.16x256b`` atoms carry split ``laneid`` iters. Without an active
    cuda target the canonicalized thread axes stay ``{laneid, wid_in_wg}``; under
    ``sctx.target`` they fuse to a single ``tid_in_wg`` chain — the form emit's
    ``align_operands_to_anchor`` path already relies on. ``_check_layout_operands_agree``
    must take ``sctx`` and run slice+canonicalize under ``sctx.target``.
    """
    import inspect

    from tvm.backend.cuda.operator.tile_primitive.common import get_st_extent
    from tvm.backend.cuda.operator.tile_primitive.elementwise._common import buffer_regions
    from tvm.backend.cuda.operator.tile_primitive.elementwise.ops.cast import CAST_OPS
    from tvm.backend.cuda.operator.tile_primitive.elementwise.reg import (
        _check_layout_operands_agree,
    )
    from tvm.backend.cuda.operator.tile_primitive.layout_utils import get_sublayout_from_region
    from tvm.tirx import TilePrimitiveCall
    from tvm.tirx.exec_scope import ExecScope
    from tvm.tirx.operator.tile_primitive import DispatchContext
    from tvm.tirx.stmt_functor import post_order_visit

    sig = inspect.signature(_check_layout_operands_agree)
    assert "sctx" in sig.parameters

    atom_layout = _tcgen05_cast_atom_layout()
    kernel = _tcgen05_cast_warpgroup_kernel()
    calls = []
    post_order_visit(
        kernel.body, lambda op: calls.append(op) if isinstance(op, TilePrimitiveCall) else None
    )
    plan, msg = CAST_OPS["cast"].parse(calls[0])
    assert msg is None and plan is not None
    ...
    sctx = DispatchContext(
        ...
        {},
        {},
        intra={"laneid": [32, 0], "wid_in_wg": [4, 0]},
        scope_kind="warpgroup",
    )

    def thread_names(br, *, use_sctx: bool):
        ...
        if use_sctx:
            ...
            sliced = get_sublayout_from_region(br.buffer.layout, br.buffer.shape, st, ext)
            canon = sliced.canonicalize()
        return {it.axis.name for it in canon.shard if it.axis.is_thread()}

    layout_brs = list(buffer_regions(plan))
    assert len(layout_brs) == 2
    for br in layout_brs:
        assert thread_names(br, use_sctx=False) == {"laneid", "wid_in_wg"}
        assert thread_names(br, use_sctx=True) == {"tid_in_wg"}

    ok, reason = _check_layout_operands_agree(plan, sctx)
    assert ok, reason

    from tvm.backend.cuda.operator.tile_primitive.elementwise.reg import is_reg_ewise

    pred_ok, pred_reason = is_reg_ewise(CAST_OPS["cast"])(calls[0], sctx)
    assert pred_ok, pred_reason
