...
# under the License.
"""Static tile scheduler (per-SM central queue) for megakernel.

Migrated from the production ``tirx_kernels.megakernel.utils.static_scheduler``.
"""
...
from .device import any_sync, atomic_add_int32, gt
from .packing import unpack_from_32bit
from .semaphore import SemaphoreBase
from .tile import TileSchedulerBase


class Semaphore(SemaphoreBase):
    def __init__(self, buffer, hardware: HardwareConfig | None = None):
        self.sem = buffer
...
class StaticTileScheduler(TileSchedulerBase):
    ...
    def __init__(self, prefix: str, exec_queue, smem_manager, debug=False, end_task_type: int = 31):
        super().__init__()
    ...
    def init(self):
        self._alloc()
        bx = T.cta_id([self.hardware.sm_count])
        tid = T.thread_id([self.hardware.num_threads])
        self.tile_idx = 0
        for k in T.serial(T.ceildiv(self.MAX_TASKS, self.hardware.num_threads)):
            idx = T.meta_var(k * self.hardware.num_threads + tid)
...
__all__ = ["Semaphore", "StaticTileScheduler"]
